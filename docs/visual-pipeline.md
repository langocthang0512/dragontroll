...
- Runtime environment panorama: `assets/environment/lost-world-panorama.webp` (1,010x540 WebP)

Animation states are `idle` (4 frames at 7 fps), `run` (6 at 12 fps), `jump` (2 at 8 fps), `fall` (2 at 8 fps), `attack` (5 at 15 fps), `damage` (2 at 10 fps), and `death` (5 at 8 fps). The selection screen cycles every state for production review. Gameplay drives locomotion, attack, and death overrides through the same atlas contract.
...
The recovery pass replaces the earlier flat block treatment with clustered modern pixel detail. Player frames use layered charcoal cloth, coral scarf/cape shapes, leather, gold trim, steel ramps, skin shadow, and selective ivory highlights. Male and female frames share every body, outfit, weapon, timing, and pose pixel; only hair pixels differ.

The Lost World renderer uses quiet multi-plane teal depth behind high-contrast terrain. Interactive surfaces carry carved stone modules, cracks, moss edges, roots, metal sockets, jewel facets, and warm highlights. Coins, enemies, falling eggs, checkpoint glyphs, mist, and foreground motes animate without changing simulation state.
