...
import { createTestArea } from "../maps/testArea";
...
export class PrototypeGameplaySystem {
  ...
  area: PrototypeArea = createTestArea();
  checkpointPulse = 0;
  landingPulse = 0;
  ...
  startOrResume(): boolean {
    ...
    this.syncState("TEST SYSTEMS READY");
    return true;
  }

  update(deltaSeconds: number): GameplayFrameEvents {
    this.frameEvents = { ...EMPTY_EVENTS };
    ...
    this.camera.follow(this.player.x + this.player.w / 2, this.area.width, deltaSeconds, 0.45);
    this.state.patch({ runFlow: this.respawn.state });
  ...
  restartRun(): void {
    this.area = createTestArea();
    this.lives.reset();
    this.gold.reset(this.area.gold);
    ...
    this.respawn.startPlay();
    this.persistRun();
    this.syncState("NEW RUN STARTED");
  }

  private updatePlay(deltaSeconds: number): void {
    if (this.input.consume("k")) {
      this.applyDamage("DAMAGE TEST  LIFE LOST");
      return;
    }
    ...
    if (movement.landed) {
      this.landingPulse = 0.14;
      this.state.patch({ message: "CLEAN LANDING" });
    }

    if (this.input.consumeAction("attack") && this.combat.requestAttack()) this.frameEvents.attackStarted = true;
    const combatResult = this.combat.update(deltaSeconds, this.player, this.area.target);
    this.frameEvents.attackHit = combatResult.hit;
    if (combatResult.hit) this.state.patch({ message: "TRAINING HIT CONFIRMED" });
    ...
    if (this.player.y > this.area.voidY) this.applyDamage("VOID FALL  LIFE LOST");
  }
  ...
  private updateRespawn(deltaSeconds: number): void {
    ...
    if (event === "respawn") {
      this.placeAtRespawn();
      this.syncState("RESPAWNED AT CHECKPOINT");
    }
    if (event === "gameOver") {
  ...
  private restoreSavedRun(): void {
    ...
    this.placeAtRespawn();
    this.syncState(this.canResume ? "RUN RESTORED" : "RUN OVER");
  }
  ...
  private placeAtRespawn(): void {
    ...
    this.camera.snapTo(this.player.x, this.area.width, 0.45);
  }
